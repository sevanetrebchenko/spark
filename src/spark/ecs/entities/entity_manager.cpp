...
namespace Spark::ECS::Entities {
    ...
    class EntityManager::EntityManagerData : public UtilityBox::Logger::ILoggable {
        ...
        private:
            ...
            std::vector<std::function<void(EntityID)>> _entityCreateCallbackFunctions;             // Callback functions for when entities get created.
            std::vector<std::function<void(EntityID)>> _entityDestroyCallbackFunctions;           // Callback functions for when entities get created.
            std::vector<std::function<void(EntityID)>> _componentAddCallbackFunctions;       // Callback functions for when entities get a component attached.
            std::vector<std::function<void(EntityID)>> _componentRemoveCallbackFunctions; // Callback functions for when entities get a component removed.
    };
